...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  - Docker + AWS 를 통한 배포 프로세스 학습 및 적용
    #### 달성률 50%
     - 이유 : 사용했던 Oracle db를 로컬 과 aws ec2인스턴스와 포트포워딩을 통한 연결을 해줬기 때문에 로컬 환경이 off 상태일떄 인스턴스 환경도 off된다는 단점이 있었습니다.
              또 코드의 수정시에 gradle 빌드 부터 docker 이미지 빌드 pull 작업을 반복해야 하는 번거로움이 있었습니다. 자동화를 빼먹은 배포는 반쪽자리 배포라고 느꼈기 때문 입니다.
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
