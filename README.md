...
## 기술 스택
- JAVA(java17)
- SpringBoot(3.x.x) + Jpa(SpringFramework)
- Thymeleaf
- JWT
- Redis
...
# 개발 과정 
- 요구 사항 분석 및 데이터베이스 테이블 구조 형성 (ERD)
  
- 애플리케이션 기본 기능 구현(CRUD)
  
- 실시간 클라이언트간 통신 서비스 구현을 위한 기술 탐색 후 적용
  
- 메세지의 특성을 고려한 읽기와 쓰기 작업에 적절한 기술 탐색 후 적용

- 멀티 쓰레드 환경을 고려한 적합한 자료구조 선택
  
- EC2인스턴스 생성 보안규칙 편집,Elastic ip할당후 배포준비 ( 인스턴스가 재시작 될 때 매번 public IP 가 달라지는 문제점 때문에 여러가지 설정 파일에서 맵핑이 제대로 되지 않았기에 고정으로 할당 )
  
- Docker-compose.yml 작성 후 환경변수 설정과 포트 맵핑 ( Docker 컨테이너가 실행 될 때 Oracle 이나 Redis 모니터링 툴들이 같이 시작되게 보장하기위함 그리고 포트 맵핑 )
...
# 회고 
 -  **AWS RDS 활용 어려움**
   솔직하게 RDS 비용이 개인이 쓰기에 너무 크다는 단점이있었기에 활용을 해보지 못했습니다. 따라서 저는 ec2 인스턴스에 docker 이미지화 된 오라클 인스턴스를 application 이미지와 같이 동작하도록 설정했었습니다.
   작동은 잘 됐었지만 서버의 스펙에 따라 읽기 쓰기 작업의 작은 지연이 있다는걸 알게 됐었고 실제로 ec2인스턴스 자체에 2가지 서비스를 한곳이 아니라 여러 인스턴스로 나누게 되면 서버의 부하도 줄어들고 관리 역시 편할수
   있을거라는 유추를 할수 있었습니다.
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 - **SOFT DELETE 삭제 방법 채택의 문제점**
    user삭제시 레코드 자체를 삭제하는게 아닌 enum 필드에서만 삭제 되었다라고 상태 업데이트를 통해 명시를 해줬었는데 , 이 부분에서 탈퇴 사용자가 오랜 기간 들어오지 않는다는 가정을 해본다면
    필요없는 레코드가 계속 존재하게 될 것입니다. 추후 많은 회원 수용시 이는 성능적인 측면에서 좋지 않은 영향을 주게 될 것 입니다.

    하지만 해당 과정을 통해 HARD DELETE 방법을 사용하지 않았기에 삭제되기 이전에 행의 주요 정보들을 Redis에 key-value 형식으로 저장 이후 Redis를 통한 user정보 복구를 할수 있게 되었습니다.
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 - **다대다 참조 문제**
   서로가 서로를 참조하려 하는 것을 끊어내는 방법을 알게 되었고, 중간 조인 테이블 생성을 통해 좀더 명시적이고 추후 작업시 수정이 가능하게 만드는 과정을 배웠고 다대다 관계를 풀어내는 방법 중 한가지를 알게되었습니다.
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 - **RESTful API 에 대한 학습**
 -  REST원칙을 알게 되었고 URL 설계, 메서드 GET,POST,PUT,DELETE 각각에 대한 표준 역할 을 배웠습니다.
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 - **Websocket API 사용을 통한 실시간 통신 학습**
   웹소켓의 연결과정에 대한 이해를 쌓았고 실시간 통신을 가능하게 만들었습니다. 실시간 통신이 가능해지자 많은 양의 데이터가 삽시간에 쌓였습니다.
   이에 대한 문제로 메세지의 송수신량 증가에 따른 데이터 처리를 위한 Redis를 학습하게 되는 계기가 되었고, 학습 후 Redis를 이용하여 많은 양의 데이터 송수신시 빠른
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 - **동시성 문제의 관찰과 다양한 해결 방법론 습득 및 해결**
   해결해 보면서 이를 해결하기위한 방법으로 트랜잭션 격리수준을 높이는 방법과 , Redis의 분산 Lock, 익스큐터를 이용한 스레드 풀 설정을 학습하게 되었고 최종적으로 Redis의 분산 Lock 을 적용하게 되었습니다.
   
   처음 겪어보는 동시성 문제를 마주할수 있었고 추후 데이터베이스의 ACID원칙에서의 C를 준수하게 되었고 ACID원칙 또한 배우게 되었습니다.
   또한 동시성 문제를 관찰하기 위한 모니터링 시스템 actuator + prometheus -> Grafana 를 배웠고
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 - **배포 프로세스 과정에서의 Linux 기초 명령어와 구조 학습**

   이를 이용하여 Docker를 통한 이미지 빌드, 컨테이너 화 과정, 구조를 배웠고 AWS EC2의 사용이 좀 더 익숙해지게 되었습니다.
...
## 🔴 동시성 문제 
   - 동시가입 1000명 시나리오를 테스트 해봤습니다. 이유는 1000명이 동시에 가입시 부하가 얼마나 걸리는지 궁금했습니다.
     시나리오는 이렇습니다.
     동일한 회원정보를 Jmeter를 이용해  /auth/register로 1000개의 요청을 보냅니다.
       ...
       **Synchronized키워드로 회원가입 블럭 감싸기** : 한번에 하나의 쓰레드만 해당 블럭에 접근가능하도록 해보았습니다. 이후 Jmeter를 이용해 다시 1000개의 요청을 보내봤습니다.
          ...
          이번 부하 테스트에서는 짧은 duration 시간 동안에 한번에 많은 요청을 보냈기에 적절한 부하테스트가 아니었다고 생각합니다.

          따라서 다음 번에는 6000명을 기준으로 삼고 10-15분 동안 점진적으로 증가시켜 어떤 지점에서 병목 현상이 일어나는지 관찰 하고 '
          수평적 확장 혹은 수직적 확장 기술에 대한 적용을 검토할 예정입니다.
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 -   ## 🟢 결과
      ...
      - 위의 사진에서 보시듯 Redis는 메세지 조회 API에서 약 0.1초의 응답 속도가 걸린 반면 RDB는 0.3초에 가까운 응답 속도가 걸렸습니다.
